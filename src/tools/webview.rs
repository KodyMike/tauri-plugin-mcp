...
pub async fn handle_get_dom<R: Runtime>(
    ...
    let result = get_dom_text(app.clone(), emit_target, timeout_secs).await;
    match result {
        Ok(dom_text) => {
...
pub async fn handle_get_page_map<R: Runtime>(
    ...
    // Verify the webview exists (supports multi-webview architectures)
    let _webview = crate::desktop::get_webview_for_eval(app, &window_label).ok_or_else(|| {
        crate::error::Error::Anyhow(format!("Webview not found: {}", window_label))
    })?;
    ...
    // THEN emit the request
    app.emit_to(&emit_target, "get-page-map", js_payload)
        .map_err(|e| {
...
pub async fn handle_get_page_state<R: Runtime>(
    ...
    app.emit_to(&emit_target, "get-page-state", serde_json::json!({}))
        .map_err(|e| {
...
pub async fn handle_navigate_back<R: Runtime>(
    ...
    app.emit_to(&emit_target, "navigate-back", js_payload)
        .map_err(|e| {
...
pub async fn handle_scroll_page<R: Runtime>(
    ...
    app.emit_to(&emit_target, "scroll-page", js_payload)
        .map_err(|e| {
...
pub async fn handle_fill_form<R: Runtime>(
    ...
    app.emit_to(&emit_target, "fill-form", js_payload)
        .map_err(|e| {
...
pub async fn handle_wait_for<R: Runtime>(
    ...
    app.emit_to(&emit_target, "wait-for", js_payload)
        .map_err(|e| {
